...
import com.randream.utils.VerifyCodeUtil;
...
@Controller
@RequestMapping("/v1")
public class CodeController {//验证码
    ...
    public void getCode(HttpServletResponse response, HttpServletRequest request) throws Exception {
        ...
        // 将验证码存入Session
        try {
            String session_verify_code = (String) session.getAttribute("SESSION_VERIFY_CODE");
            System.out.println("SESSION_VERIFY_CODE  " +
                    session_verify_code);
        } catch (NullPointerException e) {
            e.getCause().printStackTrace();
        }

        session.setAttribute("SESSION_VERIFY_CODE", objs[0]);
        // 打印验证码
        ...
        //String bufferedImageToBase64 = ImageBase64Util.BufferedImageToBase64(image);
//
        //response.getOutputStream().print(bufferedImageToBase64);
